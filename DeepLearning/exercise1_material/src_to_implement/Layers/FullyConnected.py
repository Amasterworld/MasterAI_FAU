...
class FullyConnected(BaseLayer):
    ...
    def forward(self, input_tensor):
        ...
        tep = self.weights[:-1]
        print("otep shape  ",tep.shape)
        output_tensor = np.dot(self.input_tensor , tep) + self.bias
    ...
    def backward(self, error_tensor):
        ...
        self._gradient_weights = np.vstack([self._gradient_weights, np.zeros((1, self.output_size))])

        print("self._gradient_weights shape ", self._gradient_weights.shape)
        ...
        print ("prev_error_tensor ", prev_error_tensor.shape)
